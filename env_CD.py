...
class Env(object):
    def __init__(self,worker_num,worker_capacity,task_num,grid_num):
        ...
        self.TASK_FILE= "dataset/CD/task_CD_0_24.txt"
        self.ORIGINAL_TIME=1477929600
        self.GRID_NUM=grid_num
        self.MAX_LNG = 104.185760325
        self.MAX_LAT = 30.7577531
        self.MIN_LNG = 103.9693232
        self.MIN_LAT = 30.5914011
                ...
                while move_distance>0:
                    if move_distance>=worker.remain_distance:
                        ...
                        if move_distance>task_distance:
                            ...
                            worker.remain_distance = self.get_distance(worker.lng, worker.lat, worker.schedule[0].lng,
                                                                       worker.schedule[0].lat) - (
                                                                 move_distance - worker.remain_distance)
                            worker.lng = worker.schedule[0].lng
                            worker.lat = worker.schedule[0].lat
                            worker.schedule.remove(worker.schedule[0])
                            self.complete_tasks_num += 1
