...
from src.model_DDAQN import DDAQN
from src.memory import Memory_DDAQN as Memory
# from tensorboardX import SummaryWriter
from collections import deque
import matplotlib.pyplot as plt
from src.config_DDAQN import initial_exploration, batch_size, update_target,epsilon, grid_num, device, replay_memory_capacity, lr,feature_num, sequence_length,action_space,human_flag,intervention_mode
...
def Simulation(env,epoch_num,online_net,target_net,optimizer,memory,loss_list,raward_list,q_online_list,q_target_list,QLearning,completion_rate_list):
    ...
    for epoch in range(epoch_num):
        rl_log_file = "./results/log/CD_epoch_" + str(epoch) + "_mode_" + str(intervention_mode)+ "_DDAQN_rl_log.txt"
        human_log_file = "./results/log/CD_epoch_" + str(epoch) + "_mode_" + str(intervention_mode)+ "_DDAQN_human_log.txt"
        ...
        for current_time_step in tqdm(range(env.ORIGINAL_TIME, env.ORIGINAL_TIME+86400)):
            ...
            if train_step > initial_exploration:
                ...
                loss,q_online,q_target = DDAQN.train_model(online_net, target_net, optimizer, batch)
        ...
        human_log_f.write("total_human_time: "+str(sum(human_time_list))+ " , total_human_num: "+ str(env.human_decision_num)
...
if __name__=="__main__":
    ...
    #输入6*grid_num*grid_num

      #Building a DDAQN network
    online_net = DDAQN(grid_num,feature_num, actions_num)
    target_net = DDAQN(grid_num,feature_num, actions_num)
    update_target_model(online_net, target_net)
    ...
    if os.path.exists('./src/qTable/q_table_DDAQN.pkl'):
        table = pd.read_pickle('./src/qTable/q_table_DDAQN.pkl')
    ...
    Simulation(env,epoch_num,online_net,target_net,optimizer,memory,loss_list,reward_list,q_online_list,q_target_list,QLearning,completion_rate_list)

    table.to_pickle('./src/qTable/q_table_DDAQN.pkl')
    ...
    plt.ylabel('DDAQN Loss')
    plt.xlabel('epochs')
    loc_time_loss = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
    save_path = fig_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize"+str(batch_size) +"_DDAQN_loss.png"
    # print(save_path)
    ...
    plt.ylabel('DDAQN Q value')
    plt.xlabel('epochs')
    loc_time_loss = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
    save_path = fig_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize_"+str(batch_size) +"_DDAQN_qValue.png"
    # print(save_path)
    foo_fig = plt.gcf()
    foo_fig.savefig(save_path, dpi=600, format='png')
    plt.close()
    ...
    plt.ylabel('DDAQN Reward')
    plt.xlabel('epochs')
    loc_time_reward = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
    save_path = fig_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize_"+str(batch_size) +"_DDAQN_reward.png"
    # print(save_path)
    foo_fig = plt.gcf()
    foo_fig.savefig(save_path, dpi=600, format='png')
    # plt.show()
    ...
    plt.ylabel('DDAQN completion Rate')
    plt.xlabel('epochs')
    loc_time_reward = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
    save_path = fig_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize_"+str(batch_size) +"_DDAQN_completionRate.png"
    # print(save_path)
    foo_fig = plt.gcf()
    foo_fig.savefig(save_path, dpi=600, format='png')
    # plt.show()
    ...
    q_target_data_file = data_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize_"+str(batch_size) +"_DDAQN_qTarget.txt"
    q_online_data_file = data_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize_"+str(batch_size) +"_DDAQN_qOnline.txt"
    loss_data_file=data_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize_"+str(batch_size) +"_DDAQN_loss.txt"
    reward_data_file = data_dir+data_set+"_gridNum_"+str(grid_num)+"_lr_"+str(lr)+"_batchSize_"+str(batch_size) +"_DDAQN_reward.txt"
    completion_rate_data_file = data_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_" +str(batch_size) + "_DDAQN_completionRate.txt"
