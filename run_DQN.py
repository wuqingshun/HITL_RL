...
from src.model_DQN import QNet
from src.memory import Memory_DQN as Memory
# from tensorboardX import SummaryWriter
from collections import deque
import matplotlib.pyplot as plt
from src.config_DQN import initial_exploration, batch_size, update_target,epsilon, log_interval, device, replay_memory_capacity, lr, sequence_length,action_space,human_flag,intervention_mode
import os
import pandas as pd
...
def select_action(state,target_net,epsilon,action_space):
    '''
      Select Action
     :param state: 
     :param target_net: 
...
def Simulation(env,epoch_num,online_net,target_net,optimizer,memory,loss_list,raward_list,QLearning,q_online_list,q_target_list,completion_rate_list):
    '''
    ...
    for epoch in range(epoch_num):
        rl_log_file = "./results/log/CD_epoch_" + str(epoch) + "_mode_" + str(intervention_mode)+ "_DQN_rl_log.txt"
        human_log_file = "./results/log/CD_epoch_" + str(epoch) + "_mode_" + str(intervention_mode)+ "_DQN_human_log.txt"
        ...
        for current_time_step in tqdm(range(env.ORIGINAL_TIME, env.ORIGINAL_TIME+86400)):
            if current_time_step % env.time_slice==0:
                ...
                if window_time_slice<action_space[0]:
                    divide_flag=0
                    action = select_action(state, target_net, epsilon_factor, action_space)
                elif window_time_slice>=action_space[-1]:
                    divide_flag = 1
                    action=action_space[-1]
                else:
                    action=select_action(state,target_net,epsilon_factor,action_space)
                    # if action==window_time_slice:
                ...
                memory.push(state, next_state, action_one_hot, reward,human)
                state=next_state
                train_step += 1
                if divide_flag==0:
                    window_time_slice+=1
            ...
            if train_step > initial_exploration:
                ...
                loss,q_online,q_target = QNet.train_model(online_net, target_net, optimizer, batch)
...
if __name__=="__main__":
    ...
    feature_num=6
    # num_inputs=4*grid_num*grid_num+2

    ##Building a DQN network
    online_net = QNet(grid_num,feature_num, actions_num)
    target_net = QNet(grid_num,feature_num, actions_num)
    update_target_model(online_net, target_net)
    ...
    if os.path.exists('./src/qTable/q_table_DQN.pkl'):
        table = pd.read_pickle('./src/qTable/q_table_DQN.pkl')
    ...
    table=Simulation(env,epoch_num,online_net,target_net,optimizer,memory,loss_list,reward_list,QLearning,q_online_list,q_target_list,completion_rate_list)
    table.to_pickle('./src/qTable/q_table_DQN.pkl')
    ...
    save_path = fig_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr) +"_batchSize_" +str(batch_size) + "_DQN_loss.png"
    ...
    plt.xlabel('epochs')
    loc_time_loss = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
    save_path = fig_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr) +"_batchSize_"+str(batch_size) + "_DQN_qValue.png"
    # print(save_path)
    foo_fig = plt.gcf()
    foo_fig.savefig(save_path, dpi=600, format='png')
    plt.close()
    ...
    save_path = fig_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_"+str(batch_size) + "_DQN_reward.png"
    # print(save_path)
    foo_fig = plt.gcf()
    foo_fig.savefig(save_path, dpi=600, format='png')
    # plt.show()
    ...
    plt.ylabel('DQN completion Rate')
    plt.xlabel('epochs')
    loc_time_reward = time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))
    save_path = fig_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_" +str(batch_size) + "_DQN_completionRate.png"
    # print(save_path)
    foo_fig = plt.gcf()
    foo_fig.savefig(save_path, dpi=600, format='png')
    # plt.show()
    ...
    #
    q_target_data_file = data_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_" +str(batch_size) + "_DQN_qTarget.txt"
    q_online_data_file = data_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_"+str(batch_size) + "_DQN_qOnline.txt"
    loss_data_file = data_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_"+str(batch_size) + "_DQN_loss.txt"
    reward_data_file = data_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_"+str(batch_size) + "_DQN_reward.txt"
    completion_rate_data_file = data_dir + data_set + "_gridNum_" + str(grid_num) + "_lr_" + str(lr)+"_batchSize_"+str(batch_size) + "_DQN_completionRate.txt"
